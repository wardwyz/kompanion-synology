...
func TestJoplinAPI_SearchNotes(t *testing.T) {
	gin.SetMode(gin.TestMode)
	r := gin.New()
	noteSvc := notes.NewService(notes.NewMemoryRepo())
	...
	searchReq := httptest.NewRequest(http.MethodGet, "/joplin/search?token=test-token&query=sync", nil)
	...
	if !strings.Contains(searchResp.Body.String(), "KOReader Sync Test") {
		t.Fatalf("expected matched title in search response, got %s", searchResp.Body.String())
	}
}
